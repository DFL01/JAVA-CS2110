 ...
 *
 * What I thought about this assignment: Insightful assignment about heaps.
 *
 *
 */
...
public final class Heap<T> {
    ...
    protected void ensureSpace() {
        ...
        if (size == b.length) { b= Arrays.copyOf(b, b.length * 2); }
    }
    ...
    void bubbleUp(int h) {
        ...
        if (h >= size) {} else {
            int i= (h - 1) / 2;
            while (compareTo(b[h].priority, b[i].priority) == 1) {
                swap(i, h);
                h= i;
                i= (i - 1) / 2;
            }
    ...
    void bubbleDown(int h) {
        ...
        if (rc >= size) {
            hpc= lc;
        } else if (compareTo(lc, rc) == -1) {
            hpc= rc;
        }

        while (compareTo(h, hpc) == -1) {

            swap(h, hpc);
            h= hpc;
            lc= 2 * h + 1;
            rc= lc + 1;
            if (hpc >= size || lc >= size) { return; }
            hpc= lc;
            if (rc >= size) {
                hpc= lc;
            } else if (compareTo(lc, rc) == -1) {
    ...
    public void changePriority(T v, double p) {
        ...
        // will find no errors.
        if (map.get(v) == null) { throw new IllegalArgumentException(); }
        double oldPriority= b[map.get(v)].priority;
        Pair holder= b[map.get(v)];
        holder.priority= p;
