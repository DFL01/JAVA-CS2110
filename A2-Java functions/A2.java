...
/* NetIds: df356, sau8.
 * What I thought about this assignment: Insightful assignment to read Java API.
 *
 *
 *
 *  */
...
public class A2 {
    ...
    public static double timeSpent= 4.25;
    ...
    public static String protectLittles(String s) {
        ...
        for (int i= 0; i < s.length(); i++ ) {
            if (s.substring(i, i + 1).matches(".*[abcdefghijklmonpqrstuvwxyz].*")) {
    ...
    public static String putCapsLast(String s) {
        ...
        String lower= "";
        for (int i= 0; i < s.length(); i++ ) {
            if (s.substring(i, i + 1).matches(".*[ABCDEFGHIJKLMNOPQRSTUVWXYZ].*")) {
    ...
    public static boolean exactly1(String s, String s1) {
        ...
        if (s.indexOf(s1) > -1) {
            if (s.substring(s.indexOf(s1) + 1).indexOf(s1) > -1) {
                return false;
    ...
    public static int nCat(String s, String x) {
        ...
        if (xLen == 0 && sLen != 0) { return -1; }
        if (sLen == 0) { return 0; }

        if (sLen % xLen == 0) {
            for (int i= 0; i < sLen / xLen; i++ ) {
                String subS= s.substring(i * xLen, (1 + i) * xLen);
                if (subS.equals(x)) {
                    counter= counter + 1;
                } else {
                    return -1;
    ...
    public static int shortest(String s) {
        ...
        for (int i= 1; i <= s.length(); i++ ) {
            if (nCat(s, s.substring(0, i)) > 0) { return i; }
